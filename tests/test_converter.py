...
def test_d_cnv_01_to_meter_feet(default_registry):
    """D-CNV-01 / FR-02: to_meter — 1 feet -> 0.3048 m (±ε). OCP 회귀는 green D-OCP-01."""
    converter = Converter(default_registry)
    result = converter.to_meter(1, "feet")
    assert abs(result - 0.3048) < 1e-4


def test_d_cnv_02_convert_all_meter_to_feet(default_registry):
...
def test_d_cnv_03_convert_all_feet_to_yard_via_meter(default_registry):
...
def test_d_cnv_04_convert_all_meter_to_yard(default_registry):
...
def test_d_reg_01_register_cubit(default_registry):
    """D-REG-01: register — cubit 0.4572 enables conversion."""
    default_registry.register("cubit", 1 / 0.4572)
    converter = Converter(default_registry)
